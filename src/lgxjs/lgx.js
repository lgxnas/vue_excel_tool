...
Number.prototype._toFixed = function (num = 0) {
    // console.log(this)
    let str = this.toString()
    let len = str.length
    let res = 0
    if (str.indexOf(".") != -1) {
        let floatArr = str.split(".")[1]
        //    console.log(floatArr,str[len-1])
        if (str[len - 1] == "5" && floatArr.length == num + 1) {
            let arr = str.match(/\d/g)
            if (parseInt(arr[arr.length - 2]) % 2 == 0) {
                if (num == 0) {
                    res = str.slice(0, len - 2)
                } else {
                    res = str.slice(0, len - 1)
                }
            } else {
                let newNum = parseFloat(str + "1")
                res = newNum.toFixed(num)
                // console.log("str+1:", typeof newNum, newNum)
            }
        } else {
            res = this.toFixed(num)
        }
        return res
    }
    res = this.toFixed(num)
...
function _average(s, x, countFloat = 0) { //val_s,val_x string 0.05L 或 数字 ,countFloat 小数位数 为规范数据准备 暂时没用
    ...
    if (s == "-1" && x == "-1") { //双方未监测
        av = "-1";
        //一方未监测
    } else if (s != "-1" && x == "-1") { //上游监测
        av = s;
    } else if (s == "-1" && x != "-1") { //下游监测 
        av = x;
    } else { //双方均监测
        if (s.indexOf("L") != -1 && x.indexOf("L") != -1) { //双方未检出
            av = x;
        } else if (s.indexOf("L") == -1 && x.indexOf("L") == -1) { //双方检出
            if (x.indexOf(".") != -1) {
            ...
            //上游未检出
            if (s.indexOf("L") != -1) {
                l = parseFloat(s.replace(/L|\s/g, "")) / 2;
                unl = parseFloat(x.replace(/L|\s/g, ""));
            } else { //下游未检出
...
function checkdbsValue(key, value, islake = false) {
    ...
    let lv = 0;
    let v3 = 0;
    // console.log("tyvalue:",typeof(value)) //string
    if (key == "水温") {
        // console.log(key,value)
        return { key: key, bs: 0, level: -1 }
    }
    let arrKey = [];
    for (let k in dbs[0]) {
        arrKey.push(k);
    }
    // console.log("inde:",arrKey.indexOf(key))
    if (arrKey.indexOf(key) == -1) { //不是项目，直接return level: -1
        return {
            key: key,
            bs: bs,
            level: -1
        ...
        if (value.toString().indexOf("L") != -1 || value.toString() == "-1") { //未检出，直接return level:1
            return {
                key: key,
                bs: 0,
                level: 1
            ...
            // console.log("check",value,typeof value) //number
            if (key == "PH") {
                if (value < 6 || value > 9) {
                    return {
                        key: key,
                ...
                v3 = dbs[2][key];
                // console.log("checkv3:",v3,"va:",valaue)
                for (let i = 0; i < dbs.length; i++) {
                    // console.log("dbs[3][key]:",dbs[3][key],typeof(dbs[3][key]))
                    if (value <= parseFloat(dbs[i][key])) {
                        lv = i + 1;
                        // console.log("check lv:", lv, key)
                        if (i > 2) {
                            let tmpbs = Math.abs(value - v3) / v3;
                            bs = tmpbs.toFixed(2); //toFixed 4舍6入
                        }
